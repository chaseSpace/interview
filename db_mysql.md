...
- 编程复杂性：在应用程序代码中，处理 NULL 值通常需要额外的 NULL 值逻辑判断，否则就可能导致空指针错误。
- 容易混淆含义：NULL 值和类型零值在数据库中是完全不同的概念。
- 索引效率低下：包含 NULL 值的索引列通常不能参与某些优化
    - 比如 ICP/范围查询等，因为大量的 NULL 值会大大降低索引列的选择性，也无法进行二分查找。
- 统计和聚合函数的困扰： COUNT(*) 会计算所有行，但 COUNT(column_name) 会排除所有 NULL 值。这可能导致在使用聚合函数时出现意外的结果，需要开发者时刻注意
  NULL 的特殊行为。
...
- **崩溃恢复**：只有 InnoDB 支持。数据库在异常崩溃后，数据库重新启动的时候会保证数据库恢复到崩溃前的状态。这个恢复的过程依赖于
  redo log。
- **MVCC 支持**：只有 InnoDB 支持。这是句废话，因为MVCC是事务的底层实现。多版本并发控制，是一种用于实现数据库事务并发控制的技术，可以减少加锁操作，提高性能。
...
- **设置主键**：Innodb 一定拥有一个（隐藏）主键，MyISAM 可以没有主键。
...
最左匹配原则指的是数据库在**使用复合索引**时，会优先考虑索引的最左列。只有查询条件中包含了索引的最左列（或最左几列）时，索引才能被利用。

### 什么是索引下推（ICP）

索引下推（Index Condition Pushdown, ICP）是 MySQL 5.6 引入的一项**二级联合索引**查询优化技术，旨在通过
**减少存储引擎回表的次数** 来提高查询性能。它的基本思想是将部分 WHERE 子句的条件尽量下推到存储引擎层，在存储引擎扫描索引时尽可能多地过滤掉不符合条件的记录，
...
> 之所以是这样，是因为没有 ICP 时，存储索引能力有限，它只能进行简单的索引定位（等值匹配和前缀匹配）。像 LIKE
> ‘%..’这种复杂条件超出了存储引擎在索引阶段的处理能力。
...
- 当需要访问整个行时，ICP 用于 range、ref、eq_ref 和 ref_or_null 访问方法。
- 可用于 InnoDB 和 MyISAM 表，包括分区的 InnoDB 和 MyISAM 表。
- 对于 InnoDB 表，ICP 仅用于辅助（联合）索引。
- 不支持虚拟列。
- 引用子查询的条件不支持下推。
- 引用存储函数的条件不能下推。
- 触发的条件不能下推。
...
在 Explain 语句查询结果的 Extra 列中，如果包含`Using index condition`，则表示使用了索引下推，但并不绝对。

- [验证索引下推](./mysql_docs/verify_usingindexcond.md)

**为什么有时候只有单列条件的查询也会显示 Using Index Condition**

需要明确的是，ICP 的设计初衷是为了解决复合索引的“部分利用”问题，所以 ICP 仅在查询的表上存在一个复合索引时才会被考虑。
...
- 选择合适的字段创建索引：
    - 不为 NULL 的字段：如果数据库中存储了较多的 NULL 值，会导致索性选择性很低。
...
索引合并（Index Merge）是 MySQL 引擎的一项优化技术，它允许查询在某些条件下同时利用多个单列索引，从而提高查询性能。
...
> - **不可重复读**：对其他事务正在更新的同一行数据进行 2 次（含以上）读取，2 次读到的数据可能不一致。
> - **幻读**：2 次（含以上）进行范围查询，而其他事务插入或删除了范围内的数据，那么 2 次读到的行数可能不一致。
>
...
- **临键锁**：记录锁 + 间隙锁 的组合。它锁定**一个索引记录，以及该记录之前的间隙**。
    ...
    - 例如`id between 20 and 30`（存在 id=[10,20,30]的记录），锁住的 id 范围是 (10, 20]、(20,30]。
    - 例如`id>=20`（存在 id=[10,20,30]的记录），锁住的 id 范围是 (10,20]、(20,30]、(30, +无穷大))。
    - 例如`id>=30`（存在 id=[10,20,30]的记录），锁住的 id 范围是 (20,30]、(30, +无穷大)。
...
- **意向锁**（Intent Locks）：**表级锁**。表示事务将要对表中的某些行加锁，主要作用是为了让行级锁和表级锁之间能够高效协同工作。
  ...
  - **自动管理**：意向锁由 InnoDB 存储引擎自动管理，用户不能手动申请或释放。
  - **与行锁高效协同**：假设事务 A 在某些行上持有 IX 锁（表示行正在被修改）。此时，如果事务 B 试图对整个表施加一个排他锁（X
    Lock），数据库系统只需要检查表上是否存在 IX 锁即可立即判断冲突，而无需扫描表中的每一行来确认是否有行级锁存在。
...
- **表级锁**：锁定整张表，适用于需要对整张表进行操作的场景。
  ...
  - MyISAM 存储引擎默认使用表级锁。
  - InnoDB 在执行某些 DDL 语句（如 `ALTER TABLE`）或特殊命令（如 `LOCK TABLES`）时也会使用表级锁。
  - `IS` 锁、`IX` 锁和 `AUTO-INC`锁都是表级锁。
- **行级锁**：锁定单行数据，可以允许其他事务访问不同的行，适用于高并发的应用场景。
    - 只锁定需要的行数据。开销最大，加锁慢（需要查找特定行），但并发度最高。
  - InnoDB 存储引擎默认使用行级锁。
      - 记录锁、间隙锁、临键锁都是行级锁。
- **页级锁**：锁定一个数据页（Page），通常是特定存储引擎的中间粒度（如 BDB 引擎，InnoDB 没有）。
    - 一个页通常包含多行数据。
    - 它在并发度和开销之间提供了一个折衷。
...
- **悲观锁**
  ...
  - MySQL 的大部分锁机制（如行锁、表锁）都属于悲观锁实现。使用 `FOR UPDATE` 或 `LOCK IN SHARE MODE` 显式加锁即是悲观锁的应用。
- **乐观锁**
    - 假设冲突很少发生，不依赖数据库的锁机制。它通常通过在表中增加版本号（version）或时间戳字段来实现。
  - 在更新数据时，检查当前版本号是否与读取时一致，不一致则说明有冲突，更新失败（需要重试）。MySQL 本身不提供乐观锁关键字，需要在应用层面实现。
...
Control）是一种用于实现事务和提高数据库并发性能的关键技术，主要在 InnoDB 存储引擎中实现。它通过管理数据行的多个历史版本，允许多个事务同时访问相同数据行，
从而在大多数情况下避免了读操作与写操作之间的阻塞，尤其是在读多写少的 OLTP（联机事务处理）应用中优势明显。

#### 核心目标

MVCC 的主要目标是解决读写冲突，实现非阻塞的并发读写。在没有 MVCC 的传统加锁机制（悲观锁）下，一个事务在读取数据时可能会阻止另一个事务写入同一行数据，
反之亦然，这会大大降低数据库的并发性能。
...
1. **隐藏列**：InnoDB 为每行记录添加了两个或三个隐藏的系统字段
    - TRX_ID (事务 ID)： 记录最近一次对该行数据进行修改（插入/更新）的事务 ID。
    - ROLL_PTR (回滚指针)： 指向 undo log 中该行数据的上一个版本。
    - DB_ROW_ID (行 ID)： （可选）如果表没有聚簇索引，InnoDB 会生成一个隐藏的行 ID 作为聚簇索引。
2. **Undo Log (回滚日志)**
    - Undo log 主要用于事务回滚，同时也是 MVCC 实现多版本的基础。
    - 每次数据更新操作都会生成一条对应的 undo log 记录，记录了数据修改前的值。
    - 这些 undo log 记录通过`ROLL_PTR`连接起来，形成一个版本链（Version Chain）。通过这个版本链，可以根据需要回溯到该行数据的早期版本。
3. **Read View (读视图)**
    - Read View 是在一个事务启动时或进行快照读（SELECT 操作，非锁定读）时生成的当前数据库中活跃事务的快照。
    - 它定义了当前事务能看到哪些数据版本——要么是事务启动时其他事务已提交的数据，要么是读取时其他事务已提交的数据。
    - 事务通过可见性规则来判断该版本数据是否对当前事务可见。
    - 读视图的创建时机由隔离级别决定。
...
读视图是 MVCC 算法的核心特性，它直接实现了**快照读**，而这正是 MVCC 支持高并发读写的核心机制。

**与隔离级别的关系**

隔离级别决定了读视图的创建时机。比如在 RU/SR 级别下，不会创建读视图，这是因为在 RU 隔离级别下，
查询可以看到其他事务“尚未提交的更改”，也就是最新数据，无需判断；在 SR 级别下，对相同数据的读取事务都是串行执行，
数据也就没有多版本，无需判断。只有在 RC/RR 级别下，会创建读视图。
...
1. trx_id < min_trx_id → 可见
    - 数据的事务 id 小于当前系统已分配的最小事务 ID，说明该事务在读视图生成前就已提交，自然可见。
...
- 在 RC 级别下，每次读取（一般指的是快照读）都会创建新的读视图，都可以看到其他事务已提交的数据。
- 在 RR 级别下，仅在事务启动时创建一个读视图，用于在整个事务中共享，解决了不可重复读问题。
...
快照读（不加 FOR UPDATE 和 LOCK IN Share Mode 的 SELECT 语句）是 MVCC 中的一种无锁读取机制，
它基于读视图和多版本链读取一个对当前事务“可见”的历史已提交版本。而数据更新是写最新版本，与快照读互不阻塞。

**注意：快照读依赖读视图，所以同样仅在 RC/RR 级别下存在。**

**作用**

为 RC 和 RR 级别提供高并发的数据库读取性能，减少了数据在并发读写时的锁冲突概率和阻塞时间。

##### 当前读

**定义**

当前读（加 FOR UPDATE 或 LOCK IN SHARE MODE 的 SELECT 语句，以及其他的增删改语句）是一种排他锁读取机制，
...
当前读是 InnoDB 读取记录最新版本并加锁的读方式，用于更新操作和加锁查询，确保读写一致性、避免冲突和（RR 级别下的）幻读。

> **如何防止 RR 级别下的幻读？**
>
> 通过在范围查询时添加 FOR UPDATE/LOCK IN SHARE MODE，就可以使用行锁+间隙锁来避免幻读。
...
表示 SQL 语句包含了排序需求，并且 **无法使用索引的顺序** 来完成排序，需要 Server 层执行独立的排序操作。
具体步骤是：

- 存储引擎根据 Where 条件（通常使用二级索引）返回需要的索引行以及需要的索引字段（本例的前提是 Where 条件可以利用一个二级索引）。
- 此时由于索引行不含 Order 字段，Server 层还需调用引擎层接口查询排序字段。
- Server 层拿到主键和排序字段值后，对数据行进行排序（将主键和排序字段放入**sort_buffer**）。
- Server 层将排序结果返回给客户端。

它与`Using index` 是互斥的，不会同时出现。`Using filesort` 通常是查询优化的一个重要信号，提醒我们需要优化查询 SQL 或优化索引。
...
- 且该列 不包含 NULL
- 每次索引查找必然找到 0 或 1 条记录

#### 3. range —— 范围扫描

例如 >、<、BETWEEN、LIKE 'xxx%'，使用了索引，但条件是范围。

> 范围条件会导致联合索引后续列失效。

#### 4. ref_or_null 唯一等值或 NULL 查询

这个类型比较少见，例如`col = value OR col IS NULL`。
