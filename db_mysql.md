...
- 当需要访问整个表行时，ICP 用于 range、ref、eq_ref 和 ref_or_null 访问方法。
...
3. **Read View (读视图)**
    - Read View 是在一个事务启动进行快照读（SELECT 操作，非锁定读）时生成的当前数据库中活跃事务的快照。
    - 它定义了当前事务能看到哪些数据版本——要么是事务启动时的数据，要么是读取时已提交的数据。
    - 事务通过比对数据行的 TRX_ID 和 Read View 中的规则来判断该版本数据是否可见。
   - 读视图的创建时机由隔离级别决定。
...
- 在 RC 级别下，每次读取（一般指的是无锁读）都会创建新的读视图，都可以看到其他事务已提交的数据。
