...
> 之所以是这样，是因为没有 ICP 时，存储索引能力有限，它只能进行简单的索引定位（前缀匹配）。像 LIKE
...
> **为什么说不建议为 NUll 值字段建立索引？**
>
> 因为 B+树索引的设计目的是为了高效查找非空值，B+树索引的关键优势在于其维护了键值的严格有序性。数据在索引结构中从左到右按大小顺序排列。
> 这种有序结构使得数据库可以非常快速地执行精确查找（等于）、范围查找（>、<、BETWEEN）和排序操作（ORDER BY）。
> NULL 在数学或逻辑上缺乏一个明确的、可排序的位置定义。
...
Control）是一种用于提高数据库并发性能的关键技术，主要在 InnoDB 存储引擎中实现。它通过管理数据行的多个历史版本，允许多个事务同时访问数据库，
从而在大多数情况下避免了读操作与写操作之间的阻塞，尤其是在读多写少的 OLTP（联机事务处理）应用中优势明显。

#### 核心目标

MVCC 的主要目标是解决读写冲突，实现非阻塞的并发读。在没有 MVCC 的传统加锁机制（悲观锁）下，一个事务在读取数据时可能会阻止另一个事务写入同一行数据，
反之亦然，这会大大降低数据库的并发性能。
...
隔离级别决定了读视图在具体配置的工作规则。比如在 RU/SR 级别下，不会创建读视图，这是因为在 RU 隔离级别下，
