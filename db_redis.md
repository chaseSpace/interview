...
Redis 可以使用一个 ZSet+两个 List 实现延迟队列，作用如下：
...
- 定时轮询 ZSet：每隔 2-5 秒，操作 ZSet 键，通过 Lua 脚本实现以下操作：
  - 使用`ZRANGEBYSCORE`命令取出 Score 小于等于当前时间的消息；
  - 在解包后推入另一个立即消费队列中（List 结构）。
  - 使用`ZREM`命令删除 ZSet 中已经取出的消息。
  - 返回取出的消息（自动按时间戳低到高排序）。
  - **@note**：一般不需要每秒轮询 Redis，允许几秒的误差。
- 定时轮询消费队列：阻塞监听消费队列，取出消息，执行业务逻辑。
...
Redisson 是 Java 实现的一个 22k+星星的 Redis 客户端，它不止于客户端，还基于 Redis 实现了许多功能。它的延迟队列实现基于以上思路进行了优化，
...
的客户端定时器策略需要基于一个发布订阅模式，
实现会更精确，但也更为复杂。如果使用非 Java 语言实现延迟队列，可以参考以上两种方案。
