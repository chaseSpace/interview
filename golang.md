...
有些结构体内部含状态指针的字段，生来就是不希望被复制的，复制后的使用就可能产生bug。因为当该对象被拷贝时，会使得对象中的状态指针字段变得不再安全。
...
具体的用法就是在结构体内嵌入`sync`
包成员，因为sync包内的所有类型都是不允许复制的，参考 [nocopy 示例](tests/nocopy_test.go) 。
