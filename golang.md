...
此法的缺点是整个阶段执行“stop the world”，这会导致较大延迟。另外，此法没有处理内存碎片，因为此法不做对象位置的整理，
会导致内存分配效率低。
...
**解决漏标问题**：三色标记本身存在一个问题，黑色对象仍然可能会被指向一个白色对象，从而导致这个白色对象遗漏（漏标风险）。
Go 使用写屏障来维持三色不变性（把白色对象变为灰色），从而保证并发标记的正确性并实现无须长时间 STW 的 GC。

Go GC 具体过程如下：

1. 所有对象初始为白色。
2. GC 开始时，对根对象进行标记（STW），根对象加入灰色队列。
3. 并发标记阶段：
    * 不断从灰色队列取对象
    * 扫描其引用
   * 每发现一个白色对象 → 标为灰色
...
可见扫描结束后只剩下白色和黑色两种对象。此法仍然存在内存碎片的问题，所以很多 GC 算法仍然会在此法之上添加**内存整理**的算法。
...
- 插入屏障：指针修改时，指向的新对象要标灰（无论新对象是灰是白）。
    - 这样可以确保正在引用的对象不会被当做白色误清除。
- 删除屏障：指针置空时，修改前指向的对象要标灰（无论该对象是灰是白）。
    - 为灰色说明其引用链还没有完成标记
    - 为白色说明还没有标记到此对象。若不标记为灰色，则此对象将一直是白色，并最终被清除。

引用链变化情况无法就是插入和删除两种，所以只要确保这两种情况下不发生漏标即可。此外，屏障技术都在并发标记过程中针对
**堆空间**使用，而不会对栈空间使用。写屏障通过一个全局开关（`runtime.writeBarrier.enabled`）启用，启用后，
所有堆空间中针对变量（或成员字段）的写操作都会触发写屏障（一种 Hook 机制）。
...
- 依旧默认全白色
- GC 期间新分配的对象为黑色（不可达对象仍然是白色）
    - 因为是黑色，无需再扫描全堆对象，减少 STW 时间
- 只要修改变量，在修改前就置对象为灰色
    - 不再区分修改/删除
    - 确保黑不指白，保证三色不变性
- 再标记（re-mark）阶段大幅缩短
    - 旧版 re-mark 比较复杂，因为写屏障要补偿很多不同情况。

Go1.8 的混合写屏障让 Go GC 真正进入了“低暂停、可预测、高吞吐”的时代。
...
变量的生命周期指的是变量在程序运行期间的存活的执行范围，包括了变量被声明后到被释放前的整个时间段。
...
第一种方式更常用，但它的问题是会对切片元素进行拷贝，所以修改切片元素不会影响原切片（指针除外）。
...
- nil 切片的数组内存地址是 0，空切片的数组内存地址则是一个非 0 地址
- nil 切片不可以作为 copy 函数的`dst`参数（语法正确，但无法填入数据）
    - 因为 copy 不负责对`dst`进行扩容
    - nil 切片可以 append

### new 和 make 的区别

- new 用于为任意类型分配内存，并返回指向该类型零值的指针，包括容器类型（但不负责初始化容器）。
- make 用于创建容器类型，例如数组/切片、映射和通道，返回的是值本身。
...
有些结构体内部含状态指针的字段，生来就是不希望被复制的，复制后的使用就可能产生 bug。因为当该对象被拷贝时，会使得对象中的状态指针字段变得不再安全。
所以为了安全性需要提供保护机制防止对象复制。

Go 语言中提供了两种 nocopy 检查机制，一种是在运行时检查，一种是静态检查。Go 官方目前只提供了`strings.Builder`和`sync.Cond`
的 runtime 拷贝检查机制，对于其他需要 nocopy 对象类型来说，使用 go vet 工具来做静态编译检查。

具体的用法就是在结构体内嵌入`sync`
包成员，因为 sync 包内的所有类型都是不允许复制的，参考 [nocopy 示例](tests/nocopy_test.go) 。
...
- **简化并发编程**：CSP 模型通过 channel 来进行 Goroutine 之间的通信，避免了高开销的锁操作和同步机制，使得并发编程更加简单直观。
