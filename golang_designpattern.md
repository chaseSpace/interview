...
除了GoF提出的设计模式，本篇内容还介绍了其他常见的设计模式，例如空对象模式、MVC模式等。
...
**举例**：比如业务方需要调用查询用户接口来获取用户信息，显然传入的用户ID可能不存在，此时可以选择返回一个空用户对象，
而不是返回一个Error。因为业务方可能并不关心用户是否存在，只关心用户的信息是否返回。返回一个空用户对象避免了业务方增加对Error的处理，导致冗余代码。
...
**应用**：拦截器模式主要应用于需要对请求进行拦截和处理的场景，例如权限验证、日志记录、异常处理等。例如前后端中的Web框架、RPC框架等。

**举例**：在Web框架中，拦截器可以修改请求和响应，也可以对请求和响应进行包装和处理。一个拦截器可以
