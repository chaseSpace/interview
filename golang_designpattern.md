...
本篇的前篇是[Go 设计模式篇](golang_designpattern.md)。

除了 GoF 提出的设计模式，本篇内容还介绍了其他常见的设计模式，例如空对象模式、MVC 模式等。
...
享元模式（Flyweight Pattern）是一种结构型设计模式，它用于减少创建对象的数量，从而**优化内存**的使用。这种模式特别适用于处理大量相似对象的情况，
通过共享已经存在的对象来避免创建重复的数据。
...
我们可以使用观察者模式来设计这个系统。那么这里的具体主题是天气服务，具体观察者是各个子系统（如 WebGUI 和 AlertSystem）。
...
此模式在如 Java、Python 等语言中经常使用，它们通常提供将集合类型转换为迭代器的内置方法。
...
- 集合对象：是一个包含多个元素的集合对象（如数组、哈希 Map 等），负责提供创建迭代器对象的方法。
...
**场景**：比如 Web 框架中的请求处理中间件，可以对请求进行鉴权、日志记录、风险检测等操作。
...
- Invoker：命令调用者，负责调用命令（传递到 Receiver）。
...
假设 A 对象不是我创建的，但我想要保存它的状态，以便以后可以恢复它。此时如果 A 自身没有提供快照方法的话，那就没有一种好的办法能做到这一点。
因为 A 对象可能存在外部无法访问的私有变量。所以最好的办法是 A 对象自身提供一个 `makeSnapshot和restore` 的方法，
...
- 状态（State）：是一个抽象类或接口，定义了一个接口以封装与 Context 的一个特定状态相关的行为。
- 具体状态（Concrete State）：它实现了 State 接口，并定义了与 Context 的某一个具体状态相关的行为。
...
**理解**：在一个系统中，A/B/C 需要和 E/F/G 产生合作，如果直接在 A/B/C 的代码中调用 E/F/G 的代码，那么 A/B/C 和 E/F/G
之间就会产生强耦合，且许多代码都是重复的。
此时可以新增一个中介者类，A/B/C 和 E/F/G 都不需要和对方直接交互，而是通过中介者类进行交互。
...
空对象模式（Null Object Pattern）是一种设计模式，它用于处理系统中的空值或 null 对象的情况。在许多设计中，空值是一个常见问题，
因为它们可以导致应用程序错误，如 NullPointerException。空对象模式通过提供一个默认的、无行为的对象来替代 null 值，从而简化错误处理，
并允许系统在所有情况下都不需要特别的检查就能正常运行。

空对象模式的目的是：

- 避免空引用（null pointer）异常。
- 消除系统中大量的 null 检查。
- 提供默认行为，使得系统的其他部分不需要知道实际对象和空对象之间的区别。

**举例**：比如业务方需要调用查询用户接口来获取用户信息，显然传入的用户 ID 可能不存在，此时可以选择返回一个空用户对象，
而不是返回一个 Error。因为业务方可能并不关心用户是否存在，只关心用户的信息是否返回。返回一个空用户对象避免了业务方增加对
Error 的处理，导致冗余代码。

示例：[dp_nullobject.go](design_pattern/dp_nullobject.go)

### 2. 拦截器模式

拦截器模式（Interceptor Pattern）是一种设计模式，也称为 AOP（面向切面编程）中的切面（Aspect）。
它允许在对象执行操作之前或之后，添加额外的行为。

拦截器模式的目的是：

- 解耦：将与业务逻辑无关的公共行为（如日志、事务、安全等）从业务逻辑中解耦。
- 扩展性：可以在不修改业务逻辑代码的情况下，通过增加新的拦截器来扩展系统功能。
- 流程控制：控制业务逻辑的执行流程，如在执行前进行权限检查，不满足条件时可以中断执行流程。

**应用**：拦截器模式主要应用于需要对请求进行拦截和处理的场景，例如权限验证、日志记录、异常处理等。例如前后端中的 Web 框架、RPC
框架等。

**举例**：在 Web 框架中，拦截器可以修改请求和响应，也可以对请求和响应进行包装和处理。一个拦截器可以

示例：[dp_interceptor.go](design_pattern/dp_interceptor.go)
...
- https://www.runoob.com/design-pattern/design-pattern-resources.html