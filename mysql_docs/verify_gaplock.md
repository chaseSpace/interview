...
## 正例 4：普通索引+范围查询+记录不存在

对于普通索引的范围查询，仅当记录不存在时，会锁住记录所在的间隙。若范围是`>`或`<`这样包含无穷域的情况，则会锁定对应的无穷域，无特殊。

### 事务A

```
BEGIN;
SELECT * FROM students_gap_lock WHERE score > 90 and score < 95 FOR UPDATE;
```
...
-- 不锁 [95, +inf)
UPDATE students_gap_lock SET score = 1 WHERE score=95; -- 非阻塞
INSERT INTO students_gap_lock VALUES(12, 'Dave', 96); -- 非阻塞
...
-- 不锁 91
INSERT INTO students_gap_lock VALUES(10, 'Dave', 91); -- 非阻塞
INSERT INTO students_gap_lock VALUES(10, 'Dave', 94); -- 非阻塞
```

这里你会发现锁住和不锁的范围不在预期间隙`(90,95)`内。这是因为普通索引和唯一索引有点不同。
首先我们要记住，行级锁索的是索引记录，每个二级索引记录包含的是主键id和索引值。

下面开始分析，在事务B开始前，表中的二级索引排列如下：
