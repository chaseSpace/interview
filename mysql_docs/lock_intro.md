...
行级锁（不论 S 还是 X 锁）这里有三种算法，在实际情况中由 MySQL 自动管理并使用。
...
- [验证记录锁](./verify_recordlock.md)

#### 间隙锁（Gap Lock）

锁定一个范围，但是不包含记录本身，即**开区间**，不包括双端端点（如`(1,5)`）。冲突概率大于记录锁。只存在于**可重复读（RR）**
隔离级别，目的是为了解决 RR 隔离级别下幻读的现象。此外，间隙锁只应用于非唯一索引列。唯一索引列（含主键）适用于记录锁。
...
Record Lock + Gap Lock 的组合，锁定一个范围，并且锁定记录本身，即**左开右闭区间**（如`(1,5]`）。
...
默认情况下，innodb使用三者中范围最大的**临键锁**来锁定记录，但根据实际情况来选择是否进化为记录锁还是间隙锁。
