...
在 MySQL InnoDB 中，当检测到死锁时，存储引擎会主动回滚其中一个回滚代价最小的事务（通常与修改的行数和 undo log
数量有关），以解除死锁并让其他事务继续执行。该选择并不依据 SQL 语句数量或事务执行时间。

**后续处理**

死锁一旦发生会被 InnoDB 立即处理，不会长期阻塞；但如果死锁频繁出现，会导致事务回滚增多，从而增加应用层失败重试并影响整体数据库吞吐量。

### 死锁条件

- 互斥条件：某种资源一次只允许一个进程访问
- 占有并等待：一个进程已占用一种或多种资源，并同时尝试占有其他独占型资源
- 不可抢占：若资源已被某个进程占有，则该资源不能被其他进程抢占
- 循环等待（关键）：形成一个等待环

### 检测机制

InnoDB 通过构建锁等待图来检测死锁。
锁等待图（Lock Wait Graph）是一个有向图，用来描述事务之间的锁等待关系，用于检测死锁。

- 节点（Node）：事务（Transaction）
- 边（Edge）：T1 → T2 表示 T1 正在等待 T2 持有的锁
...
每当一个事务无法获取锁（即加锁失败时）时：建立一条等待边（当前事务 → 持有该锁的事务）。然后以当前事务为起点，
通过深度优先搜索或广度优先搜索来检查是否存在环（能回到自己就存在环）。

### 死锁 vs 锁等待超时

死锁是 InnoDB 主动检测并立即处理的；锁等待超时是事务被动等待，超过阈值后才失败。

锁超时的情况中是不存在循环等待的，只是单纯事务A等待事务B释放其持续占用的锁，但事务B并不依赖事务A。

> `innodb_lock_wait_timeout` 是默认锁等待超时配置，默认值50秒。超时报错：ERROR 1205 (HY000): Lock wait timeout exceeded。
