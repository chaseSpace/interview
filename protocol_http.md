...
> [!NOTE]
> 本节内容经过笔者 @chaseSpace 亲自查证，请放心食用。如有错误，欢迎指出。
...
在 HTTP/2 中之所以 HPACK 能正常应用，是因为 HTTP/2 基于 TCP 连接，而TCP协议自带头部阻塞问题，与是否使用HPACK无关。
但是 QUIC 协议不同，它的底层协议 UDP 没有头部阻塞问题，所以我们可以优化HPACK算法来避免在应用层产生头部阻塞问题。
...
- QUIC 数据包可以包含一个或多个帧，这些帧携带控制信息和应用数据。
    - 帧的类型定义了其结构和语义，例如ACK帧用于确认接收到的数据包，STREAM帧用于传输流数据。
...
注意，其中使用密钥保护的是数据包的 Payload（有效载荷）部分，不含包头部分。
