...
- ClientHello：客户端发送一个 ClientHello 消息，其中包含客户端支持的**TLS 版本**、**加密套件列表**和 32 字节随机数（简称
  c-random）。
- 服务器连续发送三个TLS消息：
  - ServerHello：**服务器**发送一个 ServerHello 消息，其中包含选择的TLS版本、加密套件和 32 字节随机数（简称s-random）。
  - Certificate：**服务器**发送它的 SSL 证书以及公钥信息。
  - ServerHelloDone：**服务器**发送一个 ServerHelloDone 消息。
