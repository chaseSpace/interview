...
Redis 中的所有值对象都使用 `redisObject` 结构表示，这是 Redis 对象系统的核心。`redisObject` 结构定义如下：

```c
typedef struct redisObject {
    unsigned type:4;        // 对象类型：REDIS_STRING/REDIS_LIST/REDIS_HASH等
    unsigned encoding:4;    // 编码方式：如 ziplist、hashtable、skiplist 等
    unsigned lru:24;        // 记录对象的最后访问时间，用于 LRU 淘汰算法
...
Redis 通过 `redisObject` 中的 `type` 字段来区分不同数据类型：
...
1. 所有 Redis 对象都包装在 `redisObject` 结构中
2. 通过 `type` 字段判断对象的基本类型
3. 通过 `encoding` 字段确定具体的底层实现
4. 根据类型调用对应的处理函数

**命令分发机制**：

- Redis 服务器维护命令表，每个命令对应特定的处理函数
- 处理函数会检查 key 对应对象的类型
- 非法的类型操作会返回错误（如对字符串执行列表操作）

### 什么是对象共享池？

Redis 实现了对象共享池来优化内存使用，主要针对整数值对象：

**共享池机制**：

- Redis 预先创建了 -10000 到 10000 范围内的整数对象
- 这些对象在服务器启动时就创建并保存在共享池中
- 多次使用同一整数时，引用相同的对象实例
...
```c
// 在服务器启动时预创建共享对象
robj *shared.integers[REDIS_SHARED_INTEGERS];

// 创建共享整数对象
for (j = 0; j < REDIS_SHARED_INTEGERS; j++) {
    shared.integers[j] = makeObjectShared(createObject(REDIS_STRING, sdsfromlonglong(j)));
}
```
...
- Redis 使用内存分配器（如 jemalloc）管理内存
- 分配器为不同大小的内存请求分配特定大小的块
- 小对象请求可能占用较大内存块，造成内部碎片

**2. 对象生命周期差异**：

- 不同对象的生存周期不同
- 频繁的创建和删除操作导致内存空间空洞
- 原来的内存块被释放，但可能无法完全被新对象利用

**3. 数据结构特性**：

- Hash、List 等数据结构可能预留额外空间
- 预分配策略可能导致空间浪费
- 数组扩容时的内存重新分配

**4. 内存分配器算法**：

- jemalloc 等分配器使用特定算法平衡性能和内存使用
- 可能优先考虑分配速度而非内存紧凑性

**碎片率计算**：

```
内存碎片率 = Redis进程内存RSS / Redis内存使用量
```
...
jemalloc 是 Redis 默认使用的内存分配器，具有以下重要作用：
...
- `narenas`：控制并发分配的区域数量
- `tcache`：线程本地缓存，加速小对象分配
- `decay time`：控制内存释放策略

### 什么是 lazy free？

Lazy free（延迟释放）是 Redis 4.0 引入的内存管理优化机制：
...
- `UNLINK`：异步删除键，替代 `DEL`
- `FLUSHASYNC`：异步清空数据库，替代 `FLUSHDB`/`FLUSHALL`
- `DELETE` 命令在特定条件下自动使用 lazy free
...
- 对象占用内存大小超过 `lazyfree_lazy_user_del` 配置
- 对象包含的元素数量超过阈值
- 根据对象类型和编码方式判断是否适合 lazy free
...
- 实际内存释放延迟，RSS 内存可能暂时不下降
- 需要额外的后台线程资源
- 可能导致内存使用量短期上升

### big key 会带来什么问题？

Big key（大键）是指占用大量内存的 Redis 键，会带来以下问题：

**1. 阻塞主线程**：

- 删除 big key 需要大量时间
- 序列化 big key 响应客户端耗时长
- 导致其他请求等待，影响响应时间
...
- RDB 持久化时 big key 耗时长
- AOF 重写时 big key 增加时间
- 影响持久化操作的完成时间
...
- 使用 `redis-cli --bigkeys` 检测
- 使用 `SCAN` + `MEMORY USAGE` 组合检测
- 将 big key 拆分为多个小 key
- 使用 `UNLINK` 替代 `DEL` 删除

### Redis 里有哪些 O(n) 命令？为什么危险？

Redis 中存在多个时间复杂度为 O(n) 或更高复杂度的命令，这些命令在大数据集上执行时非常危险：

**1. 单键操作 O(n) 命令**：

- `LRANGE key start stop`：列表范围查询，O(N)，N 为范围大小
- `HGETALL key`：获取哈希所有字段，O(N)，N 为哈希字段数
- `SMEMBERS key`：获取集合所有成员，O(N)，N 为集合元素数
- `ZRANGE key start stop`：有序集合范围查询，O(log N + M)，M 为返回元素数
- `KEYS pattern`：查找匹配的键，O(N)，N 为数据库中键的数量

**2. 多键操作 O(n) 命令**：

- `MGET key1 key2 ...`：批量获取，O(N)，N 为键的数量
- `DEL key1 key2 ...`：批量删除，O(N)，N 为键的数量
- `SUNION key1 key2 ...`：集合并集，O(N)，N 为所有集合元素总数
- `SINTER key1 key2 ...`：集合并集，O(N*M)，N 为最小集合大小，M 为集合数量

**3. 危险性分析**：

- **阻塞主线程**：O(n) 命令在大集合上执行时间长，会阻塞其他操作
- **影响性能**：导致 Redis 响应延迟增加
- **资源消耗**：占用大量 CPU 和内存资源
- **雪崩效应**：多个 O(n) 命令可能导致服务不可用

**安全使用建议**：

- 避免在生产环境使用 `KEYS` 命令，使用 `SCAN` 替代
- 对大集合操作使用 `SSCAN`、`HSCAN`、`ZSCAN`
- 限制操作范围，使用分页查询
- 在低峰期执行批量操作
- 监控慢查询日志，识别潜在危险命令