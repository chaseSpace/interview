...
- 重传机制：包含超时重传、快速重传、SACK 方法、D-SACK 方法
    - 超时重传：当发送方发送数据段后，在等待一个 RTO（Retransmission Timeout）之后还未收到对应数据段的 ACK 报文，就会重传该数据段
    - 快速重传：当发送方收到三个相同的 DACK 报文时，立即重传丢失的报文段（而不再等待 RTO）
        - SACK（选择性确认）：通过在 ACK 报文中的【选项】字段中携带需要重传的报文段编号（可能多个），通知发送方只重传丢失的报文段，提高重传效率
    - Duplicate SACK（DACK）：比 SACK 更进一步，通过在常规 ACK 报文中的【选项】字段中携带重复收到的报文段编号，通知发送方不要再重发该报文段（可能是
      ACK 丢失了）
...
- 广播地址：支持两种类型的广播地址：子网广播地址和全局广播地址
    - 子网广播地址：本地单个网段的广播地址，例如 192.168.1.0/24 的广播地址是 192.168.1.255/24（主机标志部分的二进制全部为
      1）。
    - 全局广播地址：本地所有网段的广播地址，255.255.255.255/32。
    - 不管是子网广播地址还是全局广播地址，**都不会经过路由器转发**。
- 发送广播数据包：程序将 UDP 报文的目的地址设置为广播地址，**网络中的二层交换机**会将这个 UDP
  数据包转发给同一子网中的所有设备或局域网内所有主机（全局广播）。
- 泛洪机制：主机会将广播 IP 地址解析为广播 MAC 地址（FF:FF:FF:FF:FF:FF），交换机基于二层广播机制，将该帧转发到同一广播域内的所有端口。
